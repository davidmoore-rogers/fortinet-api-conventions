...
|---|---|---|
| login | `POST /login` form `username`, `password`, `next_link=` | 302 + cookie = success; confirm with `GET /`. Logged-out looks like: redirect to `/login`, 401/403, or a 200 whose body contains `name="password"`. |
...
| progress | poll the identity call | While flashing it also carries `erase_progress`, `write_progress`, `verify_progress`, `restart_progress`, `cur_step`, `tot_step`. Ignore `msg` and `status`. Roughly 60 s to the first progress line, erase ~3m45s, write ~3m30s, verify ~40 s on a 108F. One re-login is legitimate mid-flash. |
